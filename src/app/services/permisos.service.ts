...
export class PermisosService {
  ...
  getTipos() {
    ...
    const source$ = this.http.get(url).pipe(map(((response: any) => response.data.map((item: any) => <TiposPermiso>item))))
  ...
  getPermisos() {
    ...
    const source$ = this.http.post(url, filtro).pipe(map(((response: any) => response.data.map((item: any) => <Permiso>item))))
    return firstValueFrom(source$)
  }
